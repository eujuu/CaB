...
def visualize_boxes_and_labels_on_image_array(
    ...
    max_boxes_to_draw=20,
    min_score_thresh=.5,
    #min_score_thresh=.8, #draw box only for >80% probability
    agnostic_mode=False,
    line_thickness=4,
  ...
  for box, color in box_to_color_map.items():
    ...
    '''# draw boxes only for "person" label (below 2 code lines)
    display_str_list = box_to_display_str_map[box]
    if "person" in display_str_list[0]:'''
    # draw boxes only for classes corresponding to few labels in .pbtxt
    display_str_list = box_to_display_str_map[box]
    if "N/A" in display_str_list[0]:
      pass
    else:
